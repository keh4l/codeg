...
// message input (mobile only; on desktop the folder picker lives in the
// conversation header and the branch selector in the bottom status bar).
// ============================================================================
...
export function useConversationFolderBranchPickerVisible(
  ...
  const ownFolder = ownTab
    ? (allFolders.find((f) => f.id === ownTab.folderId) ?? null)
    : null
  // The below-composer folder+branch row is mobile-only: on desktop the folder
  // picker moved into the conversation header and the branch selector into the
  // bottom status bar. Chat-mode drafts have no resolvable folder yet, but the
  // row must still show so the folder chip (and the chat-mode item) stay
  // reachable.
  return isMobile && Boolean(ownTab && (ownFolder || ownTab.isChat))
}
  ...
  const trigger =
    variant === "header" ? (
      <button
        ...
        title={title}
        className={cn(
          "flex min-w-0 items-center rounded-sm text-sm outline-none transition-colors",
          editable
            ? "cursor-pointer text-primary hover:text-primary/80 focus-visible:ring-[3px] focus-visible:ring-ring/50"
            : "cursor-default text-muted-foreground"
        )}
      >
        <span className="max-w-[220px] truncate">{headerLabel}</span>
      </button>
    ) : (
      <Button
        variant="ghost"
